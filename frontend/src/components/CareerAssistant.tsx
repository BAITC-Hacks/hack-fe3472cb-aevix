...
export function CareerAssistant({ recs }: { recs: RecommendationsResponse }) {
  ...
  return <section className="assistant-panel" aria-labelledby="assistant-heading">
    ...
    <div className="assistant-action">
      <button className="btn ghost" onClick={explain} disabled={loading} aria-busy={loading}><Icon name="spark" size={16} />{loading ? copy.loading : failed ? copy.retry : copy.explain}</button>
    ...
    {result && <div className="assistant-response" aria-live="polite">
