...
const copy = {
  ru: {
    ...
    alreadyFinished: 'Этот квест уже завершён. Обновите обучение, чтобы увидеть актуальный список.', refreshLearning: 'Обновить обучение',
    previewMissing: 'Сотрудник ещё не открыл план этого квеста. Он появится здесь после первого открытия.',
  },
  kk: {
    ...
    alreadyFinished: 'Бұл квест аяқталған. Өзекті тізімді көру үшін оқуды жаңартыңыз.', refreshLearning: 'Оқуды жаңарту',
    previewMissing: 'Қызметкер бұл квесттің жоспарын әлі ашпаған. Жоспар алғаш ашылғаннан кейін осында көрсетіледі.',
  },
  en: {
    ...
    alreadyFinished: 'This quest is already completed. Refresh your learning to see the current list.', refreshLearning: 'Refresh my learning',
    previewMissing: 'This employee has not opened the quest plan yet. It will appear here after their first visit.',
  },
}
...
type ErrorKey = 'loadError' | 'saveError' | 'conflict' | 'missing' | 'previewMissing'
...
function QuestStepsContent({ employeeId, item, teamMode, busy, saving, onRefresh, onComplete, onOpenTeam }: Props) {
  ...
  const alreadyFinished = plan?.status === 'completed'
  const canFinish = allDone && !alreadyFinished && plan?.can_complete !== false
  ...
  async function completeStep(step: number) {
    if (readOnly || lock.current || disabled || !plan) return
    lock.current = true
    setStepBusy(step)
    setError(null)
    setSaved(false)
  ...
  async function finish() {
    if (readOnly || lock.current || disabled || !canFinish) return
  ...
  return <article className="quest-step-panel">
    <button type="button" className="quest-step-toggle" disabled={busy && !plan} aria-expanded={open} aria-controls={`${id}-body`} onClick={() => { setOpen(!open); if (!open && !plan && !loading) void load() }}>
      <span className="quest-step-summary"><strong className="quest-step-title">{plan?.title || item.title || eventTitle(item.event_id)}</strong><span className="quest-step-summary-meta">{teamMode && <span className="chip primary">{c.team}</span>}{item.status === 'overdue' && <span className="chip danger">{c.overdue}</span>}<span>{total ? `${done} / ${total} ${c.completed}` : `${progress}%`}</span>{info?.duration_hours != null && <span>{info.duration_hours} {t('hours')}</span>}</span><progress max={100} value={progress} aria-label={`${eventTitle(item.event_id)}: ${progress}%`} /></span>
      <span className="quest-step-open">{open ? c.close : c.open}<Icon name="chevron" size={16} /></span>
    </button>
    {open && <div className="quest-step-body" id={`${id}-body`} aria-busy={loading || stepBusy !== null}>
      {loading && <p className="quest-step-loading" role="status">{c.loading}</p>}
      {error && <div className="quest-step-task"><p className={error === 'previewMissing' ? 'quest-step-note' : 'quest-step-feedback'} role={error === 'previewMissing' ? 'status' : 'alert'}>{c[error]}</p>{error !== 'previewMissing' && <button className="btn ghost quest-step-action" disabled={disabled} onClick={() => void load()}>{c.retry}</button>}</div>}
      ...
      {plan && <>
        ...
        <ol className="quest-step-list">{plan.steps.map((step) => <li className={`quest-step-item${step.done ? ' is-done' : ''}`} key={step.step}><span className="quest-step-number" aria-hidden="true">{step.done ? <Icon name="check" size={15} /> : step.step}</span><div className="quest-step-content"><h4>{step.title}</h4><p>{step.description}</p>{step.done ? <span className="quest-step-done"><Icon name="check" size={13} />{c.done}</span> : !readOnly && <button className="btn ghost quest-step-action" disabled={disabled} aria-busy={stepBusy === step.step} onClick={() => void completeStep(step.step)}>{stepBusy === step.step ? c.saving : c.mark}<Icon name="check" size={14} /></button>}</div></li>)}</ol>
        {!readOnly && <><p className="quest-step-note">{c.explicit}</p>
        <div className="quest-step-footer"><p className="quest-step-note">{alreadyFinished ? c.alreadyFinished : teamMode ? allDone ? c.teamReady : c.teamPending : allDone ? c.ready : c.pending}</p>{alreadyFinished ? <button className="btn ghost" disabled={disabled} onClick={() => void onRefresh()}>{c.refreshLearning}</button> : <button className="btn" disabled={disabled || !canFinish} aria-busy={saving || stepBusy === 'finish'} onClick={() => void finish()}>{saving || stepBusy === 'finish' ? c.saving : teamMode ? c.openTeam : c.finish}<Icon name={teamMode ? 'people' : 'check'} size={15} /></button>}</div></>}
      </>}
    </div>}
  </article>
}
