...
const copy = {
  ru: {
    ...
    preview: 'Проверить совместимость', suitable: 'Подходит вашему развитию', unsuitable: 'Сейчас не подходит', suitableHint: 'Обучение помогает закрыть пробелы относительно вашей карьерной цели.', unsuitableHint: 'Обучение не соответствует текущим рекомендациям или уже пройдено.', closedHint: 'Приглашение уже недоступно. Обновите список.', details: 'Подробности рекомендации', accept: 'Учиться вместе', decline: 'Скрыть', declineWarning: 'Приглашение исчезнет только из вашего списка. Коллеги по-прежнему смогут откликнуться.', confirmDecline: 'Скрыть для меня', cancel: 'Отмена', declined: 'Приглашение скрыто из вашего списка.', accepted: 'Пара создана. Поделитесь её кодом с партнёром.',
    pairs: 'Ваши пары', pairCode: 'Код пары', openPair: 'Открыть пару', restorePair: 'Открыть по коду', restoreHint: 'Ваши пары появляются в списке автоматически. Если коллега только что принял приглашение, обновите список или откройте пару по коду.', pairReady: 'Пара создана', pairHint: 'Договоритесь с коллегой о первой встрече. Добавьте обучение в свой план, чтобы отслеживать личный прогресс.', addLearning: 'Добавить в моё обучение', added: 'Добавлено в план обучения.', addedLabel: 'В моём плане', learning: 'Открыть моё обучение', pairOpened: 'Пространство пары открыто.',
  ...
  kk: {
    ...
    preview: 'Сәйкестікті тексеру', suitable: 'Дамуыңызға сәйкес', unsuitable: 'Қазір сәйкес емес', suitableHint: 'Оқу мансап мақсатыңызға қажетті дағдыларды дамытуға көмектеседі.', unsuitableHint: 'Оқу ағымдағы ұсыныстарға сәйкес емес немесе аяқталған.', closedHint: 'Шақыру енді қолжетімсіз. Тізімді жаңартыңыз.', details: 'Ұсыныс туралы', accept: 'Бірге оқу', decline: 'Жасыру', declineWarning: 'Шақыру тек сіздің тізіміңізден жасырылады. Әріптестер оған жауап бере алады.', confirmDecline: 'Мен үшін жасыру', cancel: 'Бас тарту', declined: 'Шақыру сіздің тізіміңізден жасырылды.', accepted: 'Жұп құрылды. Кодты серіктесіңізге беріңіз.',
    pairs: 'Сіздің жұптарыңыз', pairCode: 'Жұп коды', openPair: 'Жұпты ашу', restorePair: 'Кодпен ашу', restoreHint: 'Жұптарыңыз тізімде автоматты түрде көрсетіледі. Әріптес шақыруды жаңа қабылдаса, тізімді жаңартыңыз немесе жұпты кодпен ашыңыз.', pairReady: 'Жұп құрылды', pairHint: 'Алғашқы кездесуді әріптесіңізбен келісіңіз. Жеке ілгерілеуді бақылау үшін оқуды жоспарыңызға қосыңыз.', addLearning: 'Менің оқуыма қосу', added: 'Оқу жоспарына қосылды.', addedLabel: 'Менің жоспарымда', learning: 'Менің оқуымды ашу', pairOpened: 'Жұп кеңістігі ашылды.',
...
function CollaborationContent({ employeeId, onRefresh }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      api.recommendations(employeeId), api.directory(), service.invitations(employeeId), service.ownInvitations(employeeId), api.trajectory(employeeId),
      service.teams(employeeId), service.pairs(employeeId),
    ] as const)
    if (!mounted.current || current !== generation.current) return
    const [recs, people, invites, authoredInvites, trajectory, teamResults, pairResults] = results
    ...
    if (authoredInvites.status === 'fulfilled') setOwnInvitations(authoredInvites.value)
    ...
    {tab === 'teams' ? <div className="collab-layout" id={`${uid}-teams`} role="tabpanel" aria-labelledby={`${uid}-teams-tab`}>
      ...
      <aside className="collab-sidebar">
        <section className="card"><h2>{c.invitationTitle}</h2><p className="collab-muted">{recommendations.length ? c.invitationHint : c.noQuests}</p><form className="collab-form" onSubmit={publish}>
          <label className="collab-field">{c.activity}<select aria-label={c.activity} value={inviteEvent || recommendations[0]?.event_id || ''} disabled={disabled || !recommendations.length} required onChange={(event) => setInviteEvent(event.target.value)}>{!recommendations.length && <option value="">—</option>}{eventOptions()}</select></label>
          <label className="collab-field">{c.format}<select aria-label={c.format} value={format} disabled={disabled} onChange={(event) => setFormat(event.target.value as PairFormat)}>{Object.entries(formats).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="collab-field">{c.display}<select aria-label={c.display} value={displayMode} disabled={disabled} onChange={(event) => setDisplayMode(event.target.value as 'name' | 'alias')}><option value="name">{c.name}</option><option value="alias">{c.alias}</option></select></label>
