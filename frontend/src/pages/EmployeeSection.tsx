...
export function EmployeeSection({ page, profile, recs, history, map, query, busy, onComplete, navigate }: Props) {
  ...
  return <div className="section-page">
    ...
    {page === 'recommendations' && <>
      <div className="recommendation-context"><Icon name="spark" size={22} /><div><strong>{recs.target_role} · {recs.target_grade}</strong><p>{copy.hint}</p></div><button className="text-button" onClick={() => navigate('skills')}>{c.openSkills}<Icon name="arrow" size={16} /></button></div>
      ...
      <div className="quest-page-grid">
        {recommendations.length ? recommendations.map((quest) => <QuestCard key={quest.event_id} index={recs.recommendations.indexOf(quest)} quest={quest} busy={busy !== null} saving={busy === quest.event_id} onComplete={() => onComplete(quest.event_id)} />) : <div className="card state recommendation-empty"><Icon name="search" size={30} /><p>{query.trim() || filter !== 'all' ? c.noResults : t('no_recs')}</p>{filter !== 'all' && <button className="btn ghost" onClick={() => setFilter('all')}>{copy.reset}</button>}</div>}
      </div>
    </>}
