...
export function CityPage({ profile, recs, map, navigate }: { profile: EmployeeProfile; recs: RecommendationsResponse; map: GameMap; navigate: (page: Page) => void }) {
  ...
  return <div className="grid">
    <section className="city-overview card"><div><span className="chip primary">{c.level} {map.career_level}</span><h2>{map.city_name ?? 'Career City'}</h2><p>{map.current_zone} <span aria-hidden="true">→</span> {map.target_zone} · {recs.target_role}</p>{map.center && <span className="wallet"><Icon name="leaf" size={18} /><strong>{map.center.wallet_balance.toLocaleString(lang)}</strong> {c.balance.toLowerCase()}</span>}</div><ProgressRing value={map.progress_to_next_grade} caption={t('progress')} /></section>
