...
export default function App() {
  if (/^\/hr(?:\/|$)/.test(location.pathname)) return <HrPortal />
...
function EmployeePortal() {
  ...
  useEffect(() => {
    if (location.pathname === '/') history.replaceState(null, '', `/employee${location.search}${location.hash}`)
    document.title = 'Career City · Employee'
    loadWorkspace()
  }, [])
  ...
  return (
    <>
      ...
      <main id="workspace" tabIndex={-1} className={page === 'home' ? 'home-main' : 'page-main'}>
        {listError ? <div className="card state error"><p>{t('error_backend')}</p><button className="btn" onClick={loadWorkspace}>{t('retry')}</button></div> : !ready ? <div className="skeleton" /> :
